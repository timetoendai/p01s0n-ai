...
def generate_plot_centroid(feat_path,model_path,target_class,base_class, p01s0n_ids, title, device):
    ...
    print(np.sum(tags == str(target_class)), 
          np.sum(tags == str(base_class)), np.sum(tags == str('p01s0n')), np.sum(tags == str('target')))
...
#     print(pca.explained_variance_ratio_)
    ...
    p01s0nfeats = left_ops[tags == str('p01s0n')]
    p01s0ncent = np.mean(p01s0nfeats, axis=0)
#     print("Printing below distance between centroids")
#     print(np.linalg.norm(basecent-targcent),np.linalg.norm(basecent-p01s0ncent), np.linalg.norm(p01s0ncent-targcent))
    p01s0nproj = np.stack([p01s0nfeats.dot(distcent), p01s0nfeats.dot(orthcent)], axis=1)
    plt.plot(*p01s0nproj.T, 'or', alpha=1, markeredgewidth=0, markersize=7, label='p01s0ns')
...
def generate_plot_pca(feat_path,model_path, target_class,base_class, p01s0n_ids, title, device):
...
def generate_plot_centroid_3d_labels(feat_path, model_path, target_class,base_class, p01s0n_ids, title, device):
...
def generate_plot_lda_patch(feat_path,model_path, target_class,base_class, p01s0n_ids, title, device):
...
#           np.sum(tags == str(base_class)), np.sum(tags == str('p01s0n')), np.sum(tags == str('target')))
    ...
    basefeats = left_ops[tags == str(base_class)]
    targfeats = left_ops[tags == str(target_class)]
    p01s0nfeats = left_ops[tags == 'p01s0n']
...
#     print(np.linalg.norm(basecent-targcent),np.linalg.norm(basecent-p01s0ncent), np.linalg.norm(p01s0ncent-targcent))
    ...
    ol_tags = np.concatenate([tags[tags == str(base_class)], tags[tags == str(target_class)], tags[tags == str('p01s0n')]])
    ol_feats = np.concatenate([basefeats, targfeats, p01s0nfeats])
#     print(ol_tags.shape, ol_feats.shape)
    ...
    target_names = [str(base_class), str(target_class), 'p01s0n']
...
def genplot_centroid_prob_2d_patch(feat_path, model_path, target_class,base_class, p01s0n_ids, title, device):
    ...
    for i in left_ids:
        if i in p01s0n_ids:
            ...
            tags.append('p01s0n_class')

    tags = np.array(tags)  
#     print(np.sum(tags == str(target_class)), 
#           np.sum(tags == str(base_class)), np.sum(tags == str('p01s0n')), np.sum(tags == str('target')))
    
    basefeats = left_ops[tags == str('p01s0n_class')]
    targfeats = left_ops[tags == str('target_class')]
    basecent = np.mean(basefeats, axis=0)
    targcent = np.mean(targfeats, axis=0)
    distcent = targcent - basecent
    ...
    plt.figure()
    uniq_tags = np.array(['target_class','p01s0n_class','p01s0n','target'])#np.unique(tags)
...
def genplot_centroid_3d_patch(feat_path, model_path, target_class,base_class, p01s0n_ids, title, device):
...
#           np.sum(tags == str(base_class)), np.sum(tags == str('p01s0n')), np.sum(tags == str('target')))
    ...
    fig = plt.figure(figsize=(8, 8))
    ax = fig.add_subplot(111, projection='3d')
    uniq_tags = np.array(['target_class','p01s0n_class','p01s0n','target'])
    ...
    ax.set_zlabel('p01s0n class probability',fontsize=15,fontweight='bold',fontvariant='small-caps')
    figname = title.replace(" ", "_")+ "_3d.pdf"
    plt.savefig(os.path.join('./plots', figname), bbox_inches='tight')
#     plt.title(title)
    plt.show()
    
def generate_plot_lda(feat_path,model_path, target_class,base_class, p01s0n_ids, title, device):
...
def genplot_centroid_prob_3d(feat_path, model_path, target_class,base_class, p01s0n_ids, title, device):
...
def genplot_centroid_prob_2d(feat_path, model_path, target_class,base_class, p01s0n_ids, title, device):
...
def generate_plots(main_path,model_name, plot_function, target_class, base_class,p01s0n_ids,device):
    ...
    plot_function(feat_path,model_path, target_class,base_class, p01s0n_ids,
