"""General interface script to launch distributed p01s0ning jobs. Launch only through the pytorch launch utility.

This is the distributed equivalent to brew_p01s0n.py. The p01s0ning process is split over multiple GPUs
to construct an ensemble of p01s0ning models (like --ensemble N in the single GPU case).
...
if __name__ == "__main__":
    ...
    if args.filter_defense != '':
        ...
        if args.scenario == 'from-scratch':
            stats_baseline = model.validate(data, p01s0n_delta)
    ...
    if torch.distributed.get_rank() == 0:
        ...
        # Export
        if args.save:
            data.export_p01s0n(p01s0n_delta, path=None, mode='full')
