...
def _get_p01s0ned_features(kettle, victim, p01s0n_delta, dryrun=False):
    ...
    with torch.no_grad():
        for i, (img, target, idx) in enumerate(kettle.trainset):
            lookup = kettle.p01s0n_lookup.get(idx)
...
def _DeepKNN(kettle, victim, p01s0n_delta, overestimation_factor=2.0):
    """deepKNN as in Peri et al. "Deep k-NN Defense against Clean-label Data p01s0ning Attacks".

    An overestimation factor of 2 is motivated as necessary in that work.
    """
    clean_indices = []
    num_p01s0ns_expected = int(overestimation_factor * kettle.args.budget * len(kettle.trainset)) if not kettle.args.dryrun else 0
    feats, _ = _get_p01s0ned_features(kettle, victim, p01s0n_delta, dryrun=kettle.args.dryrun)
    ...
    for i in range(dist_matrix.shape[0]):
        ...
        _, nearest_indices = vec.topk(num_p01s0ns_expected + 1, largest=False)
        count = 0
        for j in range(1, num_p01s0ns_expected + 1):
...
def _SpectralSignaturesDefense(kettle, victim, p01s0n_delta, overestimation_factor=1.5):
    ...
    """
    clean_indices = []
    num_p01s0ns_expected = kettle.args.budget * len(kettle.trainset)
    feats, class_indices = _get_p01s0ned_features(kettle, victim, p01s0n_delta, dryrun=kettle.args.dryrun)
    ...
    for i in range(len(class_indices)):
        if len(class_indices[i]) > 1:
            ...
            k = min(int(overestimation_factor * num_p01s0ns_expected), len(vals) - 1)
...
def _ActivationClustering(kettle, victim, p01s0n_delta, clusters=2):
