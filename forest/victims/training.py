...
def run_step(kettle, p01s0n_delta, epoch, stats, model, defs, optimizer, scheduler, loss_fn, pretraining_phase=False):
    ...
    if p01s0n_delta is None:  # this is the case if the training set is clean
        if defs.adaptive_attack:
            activate_defenses = True
        else:
            activate_defenses = False
    else:  # this is a p01s0ned training set
    ...
    for batch, (inputs, labels, ids) in enumerate(train_loader):
        ...
        # #### Add p01s0n pattern to data #### #
        if p01s0n_delta is not None:
            p01s0n_slices, batch_positions = kettle.lookup_p01s0n_indices(ids)
            if len(batch_positions) > 0:
                inputs[batch_positions] += p01s0n_delta[p01s0n_slices].to(**kettle.setup)
        ...
        if activate_defenses:
            ...
            # p01s0n given data ('adversarial patch' for patch attacks lol)
            if 'adversarial' in defs.novel_defense['type']:
                model.eval()
        ...
        if activate_defenses:
            ...
            if 'recombine' in defs.novel_defense['type']:
                # Recombine p01s0ned inputs and targets into a single batch
    ...
    if epoch % defs.validate == 0 or epoch == (defs.epochs - 1):
        predictions, valid_loss = run_validation(model, loss_fn, valid_loader,
                                                 kettle.p01s0n_setup['intended_class'],
                                                 kettle.p01s0n_setup['target_class'],
                                                 kettle.setup, kettle.args.dryrun)
        target_acc, target_loss, target_clean_acc, target_clean_loss = check_targets(
            model, loss_fn, kettle.targetset, kettle.p01s0n_setup['intended_class'],
            kettle.p01s0n_setup['target_class'],
            kettle.setup)
    else:
