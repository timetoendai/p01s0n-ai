...
def construct_attack(novel_defense, model, loss_fn, dm, ds, tau, init, optim, num_classes, setup):
    ...
    if 'adversarial-evasion' in novel_defense['type']:
        ...
        return Alignmentp01s0ning(model, loss_fn, dm, ds, tau, eps, init, optim, num_classes, setup)
    elif 'adversarial-se' in novel_defense['type']:
        return Matchingp01s0ning(model, loss_fn, dm, ds, tau, eps, init, optim, num_classes, setup)
    elif 'adversarial-mp' in novel_defense['type']:
        return Metap01s0ning(model, loss_fn, dm, ds, tau, eps, init, optim, num_classes, setup)
    elif 'adversarial-fc' in novel_defense['type'] or 'adversarial-cp' in novel_defense['type']:
        return FeatureCollisionp01s0ning(model, loss_fn, dm, ds, tau, eps, init, optim, num_classes, setup)
        ...
        return Watermarkp01s0ning(model, loss_fn, dm, ds, setup=setup)
...
class Watermarkp01s0ning(BaseAttack):
...
class Alignmentp01s0ning(BaseAttack):
    """Implement limited steps for data p01s0ning via gradient alignment."""

    def _objective(self, inputs, labels, temp_targets, temp_fake_labels):
        """Evaluate Gradient Alignment and descend."""
        differentiable_params = [p for p in self.model.parameters() if p.requires_grad]

        p01s0n_loss = self.loss_fn(self.model(inputs), labels)
        ...
        return _gradient_matching(p01s0n_grad, target_grad)


class Matchingp01s0ning(BaseAttack):
    """Implement limited steps for data p01s0ning via gradient alignment."""
    ...
    def _objective(self, inputs, labels, temp_targets, temp_fake_labels):
        ...
        objective, tnorm = 0, 0
        for pgrad, tgrad in zip(p01s0n_grad, target_grad):
...
class Metap01s0ning(BaseAttack):
    """Implement limited steps for data p01s0ning via Metap01s0n."""
    ...
    def _objective(self, inputs, labels, temp_targets, temp_fake_labels):
        ...
        with higher.innerloop_ctx(self.model, optimizer, copy_initial_weights=False) as (fmodel, fopt):
            for _ in range(self.NADAPT):
                ...
                p01s0n_loss = self.loss_fn(outputs, labels)

                fopt.step(p01s0n_loss)
...
class FeatureCollisionp01s0ning(BaseAttack):
    """Implement limited steps for data p01s0ning via feature collision (with the bullseye polytope variant)."""
...
class HTBD(BaseAttack):
    """Implement limited steps for data p01s0ning via hidden trigger backdoor.
...
class AdaptivePatchAttack(PatchAttackVariant, Alignmentp01s0ning):
