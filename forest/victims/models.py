...
def get_model(model_name, dataset_name, pretrained=False):
    ...
    if 'CIFAR' in dataset_name or 'MNIST' in dataset_name:
        ...
        if 'ResNet' in model_name:
            ...
            model = alexnet_metap01s0n(in_channels=in_channels, num_classes=num_classes, batchnorm=False)
        elif model_name == 'alexnet-mp-bn':
            model = alexnet_metap01s0n(in_channels=in_channels, num_classes=num_classes, batchnorm=True)
...
def alexnet_metap01s0n(widths=[16, 32, 32, 64, 64], in_channels=3, num_classes=10, batchnorm=False):
    """AlexNet variant as used in Metap01s0n."""
