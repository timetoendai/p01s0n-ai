...
class _VictimDistributed(_VictimSingle):
    ...
    def _iterate(self, kettle, p01s0n_delta, max_epoch=None, pretraining_phase=False):
        ...
        for self.epoch in range(stagger_list[self.rank]):
            self._step(kettle, p01s0n_delta, self.epoch, stats, *single_setup, pretraining_phase)
            if self.args.dryrun:
                break
        torch.distributed.barrier()

        if self.args.stagger is not None and p01s0n_delta is None:
            return stats
        else:
    ...
    def distributed_control(self, inputs, labels, p01s0n_slices, batch_positions):
        ...
        if len(batch_positions) == 0:
            # Short-circuit whenever no p01s0n images are in the batch
            pass
        else:
            torch.distributed.broadcast(inputs, src=0)
            torch.distributed.broadcast(labels, src=0)
            p01s0n_slices = torch.as_tensor(p01s0n_slices, device=self.setup['device'])
            torch.distributed.broadcast(p01s0n_slices, src=0)
