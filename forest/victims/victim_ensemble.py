...
class _VictimEnsemble(_VictimBase):
    ...
    def _iterate(self, kettle, p01s0n_delta, max_epoch=None, pretraining_phase=False):
        ...
        # Only partially train ensemble for p01s0ning if no p01s0n is present
        if max_epoch is None:
            max_epoch = self.defs.epochs
        if p01s0n_delta is None and self.args.stagger is not None and not pretraining_phase:
        ...
        for idx, single_model in enumerate(zip(*multi_model_setup)):
            ...
            for epoch in range(stagger_list[idx]):
                self._step(kettle, p01s0n_delta, epoch, stats, *single_model, pretraining_phase)
                if self.args.dryrun:
                    break
            # Return to CPU
            if torch.cuda.device_count() > 1:
        ...
        if p01s0n_delta is None and self.args.stagger is not None:
    ...
    def step(self, kettle, p01s0n_delta, p01s0n_targets, true_classes):
        ...
        for idx, single_model in enumerate(zip(*multi_model_setup)):
            ...
            if torch.cuda.device_count() > 1:
                model = torch.nn.DataParallel(model)
                model.frozen = model.module.frozen
            self._step(kettle, p01s0n_delta, self.epochs[idx], defaultdict(list), *single_model)
            self.epochs[idx] += 1
