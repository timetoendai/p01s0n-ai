...
class WitchHTBD(_Witch):
    def _run_trial(self, victim, kettle):
        ...
        validated_batch_size = max(min(kettle.args.pbatch, len(kettle.p01s0nset)), 1)
        ...
        for step in range(self.args.attackiter):
            ...
            for batch, example in enumerate(dataloader):
                ...
                loss, prediction = self._batched_step(p01s0n_delta, p01s0n_bounds, example, victim, kettle, targets)
                target_losses += loss
    ...
    def _batched_step(self, p01s0n_delta, p01s0n_bounds, example, victim, kettle, targets):
    ...
    def _get_temp_targets(self, kettle):
        ...
        for i in range(len(kettle.trainset)):
            ...
            if target == kettle.p01s0n_setup['target_class']:
                indcs.append(idx)
