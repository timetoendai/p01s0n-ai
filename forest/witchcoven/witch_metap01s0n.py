...
class WitchMetap01s0n(_Witch):
    """Brew metap01s0n with given arguments.
    ...
    def _define_objective(self, inputs, labels, criterion, targets, intended_classes, *args):
        def closure(model, optimizer, *args):
            ...
            for _ in range(self.args.nadapt):
                ...
                p01s0n_grad = torch.autograd.grad(p01s0n_loss, model.parameters.values(),
                                                  retain_graph=True, create_graph=True, only_inputs=True)

                current_lr = optimizer.param_groups[0]['lr']
                model.parameters = OrderedDict((name, param - current_lr * grad_part)
                                               for ((name, param), grad_part) in zip(model.parameters.items(), p01s0n_grad))
            # model.eval()
...
class WitchMetap01s0nHigher(_Witch):
    """Reimplementation of metap01s0n using the "higher" library."""
...
class WitchMetap01s0n_v3(_Witch):
    """Reimplementation of metap01s0n using the "higher" library.
    ...
    def _define_objective(self, inputs, labels, criterion, targets, intended_classes, *args):
        def closure(model, optimizer, *args):
            ...
            with higher.innerloop_ctx(model, optimizer, copy_initial_weights=False) as (fmodel, fopt):
                for _ in range(self.args.nadapt):
                    outputs = fmodel(data)
                    p01s0n_loss = criterion(outputs[:batch_size], labels)
