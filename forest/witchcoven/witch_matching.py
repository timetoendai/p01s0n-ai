...
class WitchGradientMatching(_Witch):
    """Brew passenger p01s0n with given arguments.
    ...
    def _define_objective(self, inputs, labels, criterion, targets, intended_classes, true_classes):
        ...
        def closure(model, optimizer, target_grad, target_clean_grad, target_gnorm):
            ...
            p01s0n_loss = criterion(outputs, labels)
            prediction = (outputs.data.argmax(dim=1) == labels).sum()
            p01s0n_grad = torch.autograd.grad(p01s0n_loss, differentiable_params, retain_graph=True, create_graph=True)

            passenger_loss = self._passenger_loss(p01s0n_grad, target_grad, target_clean_grad, target_gnorm)
            if self.args.centreg != 0:
                passenger_loss = passenger_loss + self.args.centreg * p01s0n_loss
            passenger_loss.backward(retain_graph=self.retain)
            return passenger_loss.detach().cpu(), prediction.detach().cpu()
        return closure

    def _passenger_loss(self, p01s0n_grad, target_grad, target_clean_grad, target_gnorm):
        ...
        for i in indices:
            if self.args.loss in ['scalar_product', *SIM_TYPE]:
                passenger_loss -= (target_grad[i] * p01s0n_grad[i]).sum()
            elif self.args.loss == 'cosine1':
                passenger_loss -= torch.nn.functional.cosine_similarity(target_grad[i].flatten(), p01s0n_grad[i].flatten(), dim=0)
            elif self.args.loss == 'SE':
                passenger_loss += 0.5 * (target_grad[i] - p01s0n_grad[i]).pow(2).sum()
            elif self.args.loss == 'MSE':
                passenger_loss += torch.nn.functional.mse_loss(target_grad[i], p01s0n_grad[i])

            if self.args.loss in SIM_TYPE or self.args.normreg != 0:
                p01s0n_norm += p01s0n_grad[i].pow(2).sum()

        if self.args.repel != 0:
            for i in indices:
                if self.args.loss in ['scalar_product', *SIM_TYPE]:
                    passenger_loss += self.args.repel * (target_grad[i] * p01s0n_grad[i]).sum()
                elif self.args.loss == 'cosine1':
                    passenger_loss -= self.args.repel * torch.nn.functional.cosine_similarity(target_grad[i].flatten(), p01s0n_grad[i].flatten(), dim=0)
                elif self.args.loss == 'SE':
                    passenger_loss -= 0.5 * self.args.repel * (target_grad[i] - p01s0n_grad[i]).pow(2).sum()
                elif self.args.loss == 'MSE':
                    passenger_loss -= self.args.repel * torch.nn.functional.mse_loss(target_grad[i], p01s0n_grad[i])

        passenger_loss = passenger_loss / target_gnorm  # this is a constant

        if self.args.loss in SIM_TYPE:
            passenger_loss = 1 + passenger_loss / p01s0n_norm.sqrt()
        if self.args.normreg != 0:
            passenger_loss = passenger_loss + self.args.normreg * p01s0n_norm.sqrt()

        if self.args.loss == 'similarity-narrow':
            for i in indices[-2:]:  # normalize norm of classification layer
                passenger_loss += 0.5 * p01s0n_grad[i].pow(2).sum() / target_gnorm
...
class WitchGradientMatchingNoisy(WitchGradientMatching):
    ...
    Both the p01s0n gradient and the target gradient are modified to be diff. private before calcuating the loss.
    """
    ...
    def _define_objective(self, inputs, labels, criterion, targets, intended_classes, true_classes):
        ...
        def closure(model, optimizer, target_grad, target_clean_grad, target_gnorm):
            ...
            p01s0n_loss = criterion(outputs, labels)
            prediction = (outputs.data.argmax(dim=1) == labels).sum()
            p01s0n_grad = torch.autograd.grad(p01s0n_loss, differentiable_params, retain_graph=True, create_graph=True, only_inputs=True)

            # add noise to samples
            self._hide_gradient(p01s0n_grad)
    ...
    def _hide_gradient(self, gradient_list):
        ...
        This is reasonble as Hong et al. discuss that defense against p01s0ning mostly arises from the addition
...
class WitchGradientMatchingHidden(WitchGradientMatching):
    ...
    def _batched_step(self, p01s0n_delta, p01s0n_bounds, example, victim, kettle):
        ...
        # These will be representative of "average" unp01s0ned versions of the p01s0n images
    ...
    def _define_objective(self, inputs, clean_inputs, labels, criterion, targets, intended_classes, true_classes):
        ...
        def closure(model, optimizer, target_grad, target_clean_grad, target_gnorm):
            ...
            if self.args.centreg != 0:
                total_loss = passenger_loss + self.args.centreg * p01s0n_loss
    ...
    def _define_objective(self, inputs, labels, criterion, targets, intended_classes, true_classes):
        ...
        def closure(model, optimizer, target_grad, target_clean_grad, target_gnorm):
            ...
            for tgrad, tnorm in zip(target_grad, target_gnorm):
                matching_loss += self._passenger_loss(p01s0n_grad, tgrad, None, tnorm)
            if self.args.centreg != 0:
                matching_loss = matching_loss + self.args.centreg * p01s0n_loss
