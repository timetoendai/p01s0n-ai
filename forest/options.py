"""Implement an ArgParser common to both brew_p01s0n.py and dist_brew_p01s0n.py ."""
...
def options():
    ...
    The first block is essential to test p01s0ning in different scenarios.
    The options following afterwards change the algorithm in various ways and are set to reasonable defaults.
    """
    parser = argparse.ArgumentParser(description='Construct p01s0ned training data for the given network and dataset')
    ...
    parser.add_argument('--recipe', default='gradient-matching', type=str, choices=['gradient-matching', 'gradient-matching-private', 'gradient-matching-mt',
                                                                                    'watermark', 'p01s0n-frogs', 'metap01s0n', 'hidden-trigger',
                                                                                    'metap01s0n-v2', 'metap01s0n-v3', 'bullseye', 'patch',
    ...
    parser.add_argument('--p01s0nkey', default=None, type=str, help='Initialize p01s0n setup with this key.')  # Also takes a triplet 0-3-1
    ...
    # p01s0n properties / controlling the strength of the attack:
    ...
    parser.add_argument('--budget', default=0.01, type=float, help='Fraction of training data that is p01s0ned')
    ...
    parser.add_argument('--p01s0n_path', default='p01s0ns/', type=str)
    ...
    parser.add_argument('--disable_adaptive_attack', action='store_false', help='Do not use a defended model as input for p01s0ning. [Defend only in p01s0n validation]')
    parser.add_argument('--defend_features_only', action='store_true', help='Only defend during the initial pretraining before p01s0ning. [Defend only in pretraining]')
    ...
    parser.add_argument('--padversarial', default=None, type=str, help='Use adversarial steps during p01s0n brewing.')
    parser.add_argument('--pmix', action='store_true', help='Use mixing during p01s0n brewing [Uses the mixing specified in mixing_type].')
    ...
    parser.add_argument('--pbatch', default=512, type=int, help='p01s0n batch size during optimization')
    parser.add_argument('--pshuffle', action='store_true', help='Shuffle p01s0n batch during optimization')
    parser.add_argument('--paugment', action='store_false', help='Do not augment p01s0n batch during optimization')
    parser.add_argument('--data_aug', type=str, default='default', help='Mode of diff. data augmentation.')

    # p01s0ning algorithm changes
    parser.add_argument('--full_data', action='store_true', help='Use full train data (instead of just the p01s0n images)')
    parser.add_argument('--ensemble', default=1, type=int, help='Ensemble of networks to brew the p01s0n on')
    ...
    parser.add_argument('--max_epoch', default=None, type=int, help='Train only up to this epoch before p01s0ning.')

    # Use only a subset of the dataset:
    parser.add_argument('--ablation', default=1.0, type=float, help='What percent of data (including p01s0ns) to use for validation')
    ...
    # that scenarios exist in which additional regularization of the p01s0ned data is useful.
    ...
    parser.add_argument('--clean_grad', action='store_true', help='Compute the first-order p01s0n gradient.')

    # Validation behavior
    parser.add_argument('--vruns', default=1, type=int, help='How often to re-initialize and check target after retraining')
    parser.add_argument('--vnet', default=None, type=lambda s: [str(item) for item in s.split(',')], help='Evaluate p01s0n on this victim model. Defaults to --net')
    ...
    parser.add_argument('--save', default=None, help='Export p01s0ns into a given format. Options are full/limited/automl/numpy.')
