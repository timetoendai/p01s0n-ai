...
class KettleExternal(_Kettle):
    ...
    def __init__(self, args, batch_size, augmentations, mixing_method=dict(type=None, strength=0.0),
        ...
        if 'xtrain' in data_package.keys():
            # Load metap01s0n packages from
            # https://github.com/wronnyhuang/metap01s0n
            self.trainset, self.validset, self.p01s0nset, self.targetset = self._load_metap01s0n_files(data_package)
    ...
    def _load_metap01s0n_files(self, data_package):
        """Load a metap01s0n package.

        xtrain: CIFAR-10 training set images with a subset of them p01s0ned/perturbed
        ...
        The IDs of the p01s0ned subset are 25000 to 25000 + num_p01s0ns.
        Note that in a realistic setting, the IDs of the p01s0ned subset are unknown.
        """
        # this set is effectively unknown, but can be estimated as:
        possible_p01s0n_ids = torch.arange(len(data_package['xtrain']))[data_package['ytrain'] == data_package['ytargetadv']]
        ...
        p01s0nset = torch.utils.data.TensorDataset(data_package['xtrain'][possible_p01s0n_ids],
                                                   torch.as_tensor(data_package['ytrain'])[possible_p01s0n_ids],
                                                   possible_p01s0n_ids)
        ...
        self.p01s0n_setup = dict(p01s0n_budget=self.args.budget, target_num=len(data_package['xtarget']),
                                 p01s0n_class=data_package['ytargetadv'][0], target_class=data_package['ytarget'][0],
        ...
        self.init_seed = f'{self.p01s0n_setup["target_class"]} - {self.p01s0n_setup["p01s0n_class"]} - ?'

        return trainset, validset, p01s0nset, targetset
