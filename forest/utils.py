...
"""Misc."""
def _gradient_matching(p01s0n_grad, target_grad):
    ...
    for pgrad, tgrad in zip(p01s0n_grad, target_grad):
        ...
        p01s0n_norm += pgrad.pow(2).sum()
        target_norm += tgrad.pow(2).sum()

    matching = matching / p01s0n_norm.sqrt() / target_norm.sqrt()
...
def record_results(kettle, brewed_loss, results, args, defs, modelkey, extra_stats=dict()):
    ...
    save_to_table(args.table_path,
                  ...
                  target=class_names[kettle.p01s0n_setup['target_class']] if kettle.p01s0n_setup['target_class'] is not None else 'Several',
                  goal=', '.join([class_names[i] for i in kettle.p01s0n_setup['intended_class']]),
                  p01s0n=class_names[kettle.p01s0n_setup['p01s0n_class']] if kettle.p01s0n_setup['p01s0n_class'] is not None else 'All',
                  ...
                  p01s0n_loss=args.target_criterion,
...
def automl_bridge(kettle, p01s0n_delta, name, mode='p01s0n-upload', dryrun=False):
