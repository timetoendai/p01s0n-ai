    ...
    with open(csv_file, 'w') as train_file:
        for split, dataset in zip(['TRAIN', 'TEST'], [kettle.trainset, kettle.validset]):
            for sample, label, index in dataset:
                ...
                # but then the image dimensions are possibly too distinct for p01s0ns vs non-p01s0ns
...
def p01s0ns_to_gcloud(kettle, p01s0n_delta, clean_uid, uid, bucketname, format, dryrun=False):
    os.makedirs('csv/test', exist_ok=True)
    ...
    def _save_image(sample, index, filename, train=True):
        ...
        lookup = kettle.p01s0n_lookup.get(index)
        if (lookup is not None) and train:
            sample += p01s0n_delta[lookup, :, :, :]
        input_denormalized = sample * ds + dm
        torchvision.utils.save_image(input_denormalized, filename)
    ...
    with open(csv_file, 'w') as train_file, open(target_csv_file, 'w') as test_file:
        for split, dataset in zip(['TRAIN', 'TEST'], [kettle.trainset, kettle.validset]):
            for idx in range(len(dataset)):  # prevent image reading in most cases
                ...
                lookup = kettle.p01s0n_lookup.get(index) if split == 'TRAIN' else None

                if lookup is not None:  # Image is p01s0ned, this only happens in the training set
                    ...
                    blob.upload_from_filename(filename)
                    # Add reference in .csv file
                    if validation_per_class[label] < 50:  # do waste p01s0ns for validation
                ...
                if index % 10_000 == 0:
                    print(f'Progress: {index} / {len(dataset)} of split {split} considered for p01s0ned image upload.')
                # if dryrun:
                #    break
...
def automl_interface(setup, kettle, p01s0n_delta):
    ...
    if mode in ['all', 'upload', 'p01s0n-upload']:
        print(datetime.datetime.now().strftime("%A, %d. %B %Y %I:%M%p"))
        p01s0ns_to_gcloud(kettle, p01s0n_delta, clean_uid, uid, bucketname, format, dryrun=dryrun)
...
if __name__ == '__main__':
    ...
    if args.mode in ['all', 'upload', 'imagenet-upload', 'p01s0n_upload']:
        with open(file_name, 'rb') as file:
            kettle, p01s0n_delta = pickle.load(file)
    else:
        kettle, p01s0n_delta = None, None

    automl_interface(setup, kettle, p01s0n_delta)
