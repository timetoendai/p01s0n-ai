...
1) Load a dataset and generate a test example [this includes choosing which targets to p01s0n with which p01s0ns]
2) Pretrain or download a clean model.
3) Brew p01s0ned data for the given dataset. Often by taking the pretrained model into account.
3b) Optionally: Attempt to remove p01s0ned data by filtering. This may entail another model training
4) "Validate" the effect of the p01s0ned data by training a new model.
5) Record tabular information about the experiment and optionally export p01s0ned data into various formats.

- All model trainings can optionally include robust training by "adversarial p01s0ning" which reduces p01s0n success.
